...
<div id="booking_table_div">
	...
	<ul style="margin: 50px 0 0 90px;">
 	...
 	<table style="margin-left: 90px; margin-top: 10px;">
		...
		<tbody>
			<c:choose>
				<c:when test="${not empty bookingRefundList}" >
					<c:forEach var="bookingList" items="${bookingRefundList}">
				...
				<c:otherwise>
					<tr>
						<td colspan="11">환불완료 리스트가 없습니다.</td>
					</tr>
				</c:otherwise>
			</c:choose>
		</tbody>
			...
			<ul id="pagination">
				...
				<c:forEach var="pageNum" begin="${pageVO.section*10-9}" end="${pageVO.section*10}" step="1">
					...
					<c:if test="${pageVO.section >= 2 && pageNum == (pageVO.section*10-9)}">
						<li><a href="/admin/manage/booking/bookingRefundList?page=${(pageVO.section-1)*10}">&lt;</a></li>
					</c:if>
												
					<c:choose>
						<%-- page와 pageNum이 같은 경우, 페이지 번호를 강조하여 현재 페이지임을 표기 --%>		
						<c:when test="${pageNum == pageVO.page}">
							<li id="active"><a href="/admin/manage/booking/bookingRefundList?page=${pageNum}">${pageNum}</a></li>				
						</c:when>
						
						<%-- 현재 페이지가(pageNum) maxPage 이하이면 페이지 번호 생성 --%>
						<c:when test="${pageVO.maxPage >= pageNum }">
							<li><a href="/admin/manage/booking/bookingRefundList?page=${pageNum}">${pageNum}</a></li>										
						</c:when>					
					</c:choose>						
			
					<%-- next표시 --%>
					<c:if test="${pageVO.maxSection > pageVO.section && pageNum == pageVO.section*10}">
						<li><a href="/admin/manage/booking/bookingRefundList?page=${pageVO.section*10+1}">&gt;</a>
					</c:if>					
				</c:forEach>	
			</ul>				
		</c:if>
