...
<script>
	$(function() {
		$(".reserv_refund_btn").click(function() {
			...
			if (!confirm("예약번호 : " + $("#b_no").val() + "\n파티룸 환불 하시겠습니까?")) {
	        	...
	        	$.ajax({
					url : "/booking/bookingCancelComplete",  
					type : "post",            
					data : {b_no : $("#b_no").val()},
					dataType : "text",
					error : function() { 
...
<div id="booking_table_div">
	...
	<ul style="margin: 50px 0 0 90px;">
 	...
 	<table style="margin-left: 90px; margin-top: 10px;">
		...
		<tbody>
			<c:choose>
				<c:when test="${not empty bookingRefundWaitingList}" >
					<c:forEach var="bookingList" items="${bookingRefundWaitingList}">
						<tr style="font-size: 13px;" data-reservnum="${bookingList.b_no}" data-reservdate="${bookingList.b_date}">
							...
							<td>
								...
								<br>
								<button class="reserv_refund_btn" style="color:red; font-size: 13px;">환불완료</button>
							</td>
						</tr>
					</c:forEach>
				</c:when>
				<c:otherwise>
					<tr>
						<td colspan="11">환불요청 리스트가 없습니다.</td>
					</tr>
				</c:otherwise>
			</c:choose>
		</tbody>
			...
			<ul id="pagination">
				...
				<c:forEach var="pageNum" begin="${pageVO.section*10-9}" end="${pageVO.section*10}" step="1">
					...
					<c:if test="${pageVO.section >= 2 && pageNum == (pageVO.section*10-9)}">
						<li><a href="/admin/manage/booking/bookingRefundWaitingList?page=${(pageVO.section-1)*10}">&lt;</a></li>
					</c:if>
												
					<c:choose>
						<%-- page와 pageNum이 같은 경우, 페이지 번호를 강조하여 현재 페이지임을 표기 --%>		
						<c:when test="${pageNum == pageVO.page}">
							<li id="active"><a href="/admin/manage/booking/bookingRefundWaitingList?page=${pageNum}">${pageNum}</a></li>				
						</c:when>
						
						<%-- 현재 페이지가(pageNum) maxPage 이하이면 페이지 번호 생성 --%>
						<c:when test="${pageVO.maxPage >= pageNum }">
							<li><a href="/admin/manage/booking/bookingRefundWaitingList?page=${pageNum}">${pageNum}</a></li>										
						</c:when>					
					</c:choose>						
			
					<%-- next표시 --%>
					<c:if test="${pageVO.maxSection > pageVO.section && pageNum == pageVO.section*10}">
						<li><a href="/admin/manage/booking/bookingRefundWaitingList?page=${pageVO.section*10+1}">&gt;</a>
					</c:if>					
				</c:forEach>	
			</ul>				
		</c:if>
