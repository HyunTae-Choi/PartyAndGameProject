...
public class FileUploadUtil {
	...
	public static String fileUpload(MultipartFile file, HttpServletRequest request, String fileName) throws IOException {
		...
		if(org_name != null && (!org_name.equals(""))) {
			real_name = fileName + "_" + org_name;
			
			String docRoot = request.getSession().getServletContext().getRealPath("/uploadimage/" + fileName);
			makeDir(docRoot);
	...
	public static void fileDelete(String fileName, HttpServletRequest request) throws IOException{
		...
		String docRoot = request.getSession().getServletContext().getRealPath("/uploadimage/"+dirName);
	...
	public static String makeThumbnail(String fileName, HttpServletRequest request) throws Exception{
		String dirName = fileName.substring(0, fileName.indexOf("_")); 
		// 이미지가 존재하는 폴더 추출
		String imgPath = request.getSession().getServletContext().getRealPath("/uploadimage/"+dirName);
